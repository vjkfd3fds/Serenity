game.GameOverScreen = me.ScreenObject.extend({
    ...
    onResetEvent: function() {
        ...
        this.handler = me.event.subscribe(me.event.KEYDOWN, function(action, keyCode, edge) {
            if (action === "enter") {
                // Show the high score input prompt
        ...
        // Create a simple HTML button for score input
        var button = document.createElement('button');
        button.innerText = 'Input Score';
        button.style.position = 'absolute';
        button.style.left = '50%';
        button.style.top = (me.game.viewport.height / 2 + 100) + 'px';
        button.style.transform = 'translateX(-50%)';
        button.onclick = this.showHighScorePrompt.bind(this);

        // Append the button to the game container
        me.game.world.container.appendChild(button);
    },

    showHighScorePrompt: function() {
        var userHighScore = prompt("Enter your earned score (not exceeding 100):");

        if (userHighScore !== null && userHighScore !== "") {
            var highScore = parseInt(userHighScore);

            if (!isNaN(highScore) && highScore >= 0 && highScore <= 100) {
                window.open("score.html?score=" + highScore, "_blank");
            } else {
